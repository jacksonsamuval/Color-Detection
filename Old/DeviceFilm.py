    ...
    while True:
        for stream in streams:
            ...
            if frame is not None:
                processed, detected_colors = detect_colors(frame.copy())
                window_name = f"Camera {stream.index}"
                cv2.imshow(window_name, cv2.resize(processed, (480, 360)))

                # Print detected colors live
                if detected_colors:
                    print(f"🎥 Camera {stream.index}:")
